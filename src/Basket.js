    ...
    return (
      <div>
        ...
        <ul>
          {userlist.length === 0 && (
            <h1
              onClick={handleChangeBasketShop}
              className="text-5xl text-black font-bold dark:text-green-200 hover:underline"
            >
              Add first item
            </h1>
          )}
        ...
        {getTotal() > 0 && (
          <h1 className="text-3xl text-black dark:text-green-200 font-bold py-10">
            Total: {getTotal()}
          </h1>
        )}
        {getTotal() > 0 && (
          <button
            className="mr-8 text-2xl text-black bg-slate-500 dark:bg-green-200 dark:text-slate-500 rounded-full px-4 py-2 mt-8"
            onClick={() =>
              alert(`thanks for the purchase, ${getTotal()} dollars from you`)
            }
          >
            Buy
          </button>
